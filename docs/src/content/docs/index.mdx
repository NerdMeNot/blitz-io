...
template: splash
hero:
  tagline: Tokio-style async I/O for Zig. Lightweight tasks, serious throughput.
  actions:
    - text: Get Started
      ...
      variant: primary
    - text: View on GitHub
      link: https://github.com/NerdMeNot/blitz-io
      icon: external
---
...
blitz-io's architecture is ported from Tokio, the Rust async runtime with years of production edge-case fixes. Every scheduler optimization, every synchronization primitive, every platform-specific workaround has been studied and adapted for Zig's semantics.
