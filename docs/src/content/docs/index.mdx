...
<CardGrid stagger>
  ...
  <Card title="Stackless by Choice" icon="seti:zig">
    Each task is a compiler-generated state machine at ~256-512 bytes, not a full coroutine stack (16-64KB). Stackful coroutines offer nicer ergonomics, but stackless wins where it counts: predictable memory, cache-friendly layout, and millions of concurrent tasks without blowing through RAM. Same tradeoff Tokio made -- we side with Tokio.
  </Card>
</CardGrid>
...
| Coroutine model | Stackless by choice (~256-512 B/task) | Stackful (growable stacks) | N/A | TBD |
